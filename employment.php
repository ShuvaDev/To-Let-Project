...
<?php include "inc/sidebar.php";?>
    ...
    <div class="advertisement">
        <?php
            $query="SELECT tbl_post.*,tbl_category.cat_id FROM tbl_post INNER JOIN tbl_category ON tbl_post.cat=tbl_category.id AND tbl_category.cat_id=3";
            $result=$db->select($query);
            if($result!=false)
            {
                $total_row=mysqli_num_rows($result);
            }
            $query="SELECT tbl_post.*,tbl_category.cat_id FROM tbl_post INNER JOIN tbl_category ON tbl_post.cat=tbl_category.id AND tbl_category.cat_id=3 LIMIT $start_form, $per_page";
            $result=$db->select($query);
            if($result!=false)
            {
